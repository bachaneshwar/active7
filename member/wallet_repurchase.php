...
if($_POST[submit]!="")
{
	...
	if($_POST[pass]==$pass)
	{
		...
		$zsql7="SELECT * FROM  `admin_wallet_repurchase` WHERE `walletid`='1'";
		$zres7=mysqli_query($con,$zsql7);
		$zrow7=mysqli_fetch_array($zres7);
		$walletamt=$zrow7[walletamt]+$payment_amt;
		...
		if($payment_amt<=$balance_chk)
		{
			...
			$sql71="UPDATE `ewallet` SET `repurchase_amt`='$ewallet_repurchase' WHERE `spid`='$_SESSION[spid]'";
			$res71=mysqli_query($con,$sql71);

			$sql8="INSERT INTO `wallet_transfer_details`(`spid`,`amount`,`payment_dt`)VALUES('$_SESSION[spid]','$payment_amt','$today')";
			$res8=mysqli_query($con,$sql8);
			
			$sql71="UPDATE `admin_wallet_repurchase` SET `walletamt`='$walletamt' WHERE `walletid`='1'";
			$res71=mysqli_query($con,$sql71);

			
			$msg="Amounts are Successfully Transfered&st=5";
			echo '<META HTTP-EQUIV="Refresh" Content="0; URL=wallet_repurchase.php?msg='.$msg.'">';
		}
		else
		{
			$msg1="You don't have enough amount.&st=5";
			echo '<META HTTP-EQUIV="Refresh" Content="0; URL=wallet_repurchase.php?msg1='.$msg1.'">';
		}
	}
	else
	{
