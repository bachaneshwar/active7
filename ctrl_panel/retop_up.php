...
//include_once("include/find_parent.php");
...
if($num>0)
   ...
   {
   	if($nq1==1){
$area="select * from `member_topup` where `spid`='$_POST[spid]' ORDER BY `member_topup_id` DESC LIMIT 1";
$area1=mysqli_query($con,$area);
$area2 = mysqli_fetch_array($area1);
$value_det9=$my->total_row($con,package,package_id,$area2[package_id]);
$value_det10=$my->total_row($con,package,package_id,$_POST[package_id]);


if($value_det10[package_amount]>$value_det9[package_amount]){


$value_det=$my->total_row($con,package,package_id,$_POST[package_id]);
$value_det[package_amount];
$sql2="INSERT INTO `member_topup` (`spid`,`package_id`,`topup_dt`)VALUES('$_POST[spid]','$_POST[package_id]','$date')";
$rq2=mysqli_query($con,$sql2);

$query="UPDATE `member` SET `member_status`='1',`update_dt`='$date',`package_id`='$_POST[package_id]' WHERE `spid`='$_POST[spid]'";	
...
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'msg=Sucessfully Top Up.">';
...
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'msg=Please Choose Greater Amount.">';
...
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'msg=Please Type Correct Member ID">';
...
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'msg=Please type correct Serial No / E-Pin">';
...
<i class="fa fa-list"></i>Re TopUp Package
</div>
</div>
...
$area_det=$my->search_row2($con,package,st,1,package_type,1);
