...
$sql="SELECT * FROM `staged_payout`  WHERE `endt`='$_REQUEST[endt]' GROUP BY `endt`";
...
<title>Stage D Payout Report From <?=$row[stdt]?> To <?=$row[endt]?></title>
...
<div class="">
<h1>Stage D Payout Report From <?=$row[stdt]?> To <?=$row[endt]?></h1>
...
$sel1 = "select * from `staged_payout` WHERE `endt`='$_REQUEST[endt]'  AND `total_amt`!='0' ORDER BY `payout_id` ASC";
...
<td style='font-size:13px;border:1px solid black;text-transform:uppercase'><?=$row_pay[planfive_id]?></td>
...
<center> <a style='text-decoration:none;font-size:20px'  href="show_all_payout4.php">BACK</a> </center>
