...
$sql="SELECT * FROM `amount_rate` WHERE `id`='1'";
$res=mysqli_query($con,$sql);
$row=mysqli_fetch_array($res);
$joining_amt= $row[amount]+$row[Sgst]+$row[Cgst];
if($_POST[call_submit]!=""){
$sq="UPDATE `amount_rate` SET `amount`='$_POST[amount]',`Sgst`='$_POST[Sgst]',`Cgst`='$_POST[Cgst]' WHERE `id`='1'";
$qu=mysqli_query($con,$sq);
 echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$log_url.'?msg=Successfully Updated&lid=79">';
}
...
<h1>Settings</h1>
<small>Company Payout Amount Rates</small>
</div>
</section>
...
<i class="fa fa-list"></i> Company  Amount Rates  &nbsp;&nbsp;&nbsp; <b style="color:green;">Joining Amount:&nbsp;&nbsp;<?=$joining_amt;  ?> </b>
</div>
</div>
...
<tr>
<td ><B>Amount :</B></td>
<td><input  type="text" class="form-control" name="amount" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"   value="<?=$row[amount]?>"  required /> </td>
</tr>
<tr ><td colspan="2"><br/></td></tr>
