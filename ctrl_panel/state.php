...
$value_num=$my->second_num($con,state,state_id,$decode_id,st,1);

if($value_num>0){
$value_det=$my->total_row($con,state,state_id,$decode_id);
}
...
$value_det=$my->total_row($con,state,state_id,$decode_id);
if($value_det[st]==1){
$my->normal_UPDATE1($con,state,st,0,state_id,$decode_id);
}else{
$my->normal_UPDATE1($con,state,st,1,state_id,$decode_id);
}
...
$my->new_modify($con,state,$nurl,$_POST[subId]);
$nurl="$log_url?pg=$_GET[pg]&lid=$dtls_link[sublink_id]";
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'">';		
}else{
$nurl="$log_url?lid=$dtls_link[sublink_id]";
$my->new_insert($con,state,$nurl);
}
...
?>
<option value="<?=$v1['country_id']?>" <? if($v1['country_id']==$value_det[country_id]){?>Selected<?}?>><?=$v1['country_name']?></option>
<?
}
?>
</select>
...
<input type="text" class="form-control" name="state_name" placeholder="Enter Short description" value="<?=$value_det[state_name]?>"  onkeyup="showResult(this.value)"  required>
<div id="livesearch"></div>
</div>

<div class="form-group">

<input type="hidden" name="subId" value="<?=$value_det['state_id']?>" />
...
<tbody>
<?php
$msgsql="SELECT * FROM `state`  ORDER BY  `state_name` ASC limit $lower,$upper";
$msgres=mysqli_query($con,$msgsql);
while($msgrow=mysqli_fetch_array($msgres)){
$c++;

$decode_val=base64_encode($msgrow[state_id]);
...
$search=$my->total_row($con,country,country_id,$msgrow[country_id]);
...
if($pg!=1)
print "<A HREF='state.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='state.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='state.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
