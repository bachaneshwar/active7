...
$lower=$upper*($pg-1);
if($_POST[agent_code]){
$sql="select * from `sales_agent` WHERE `st`='1' AND `agent_code`='$_POST[agent_code]'";
}else{
$sql="select * from `sales_agent` WHERE `st`='1'";
...
<?php
if($_POST[agent_code]){
$msgsql="select * from `sales_agent` WHERE `st`='1' AND `agent_code`='$_POST[agent_code]' ORDER BY `agent_id` ASC limit $lower,$upper";
}else{
$msgsql="select * from `sales_agent` WHERE `st`='1' ORDER BY `agent_id` ASC limit $lower,$upper";
...
<td><?=$msg?></td>
<td>
<a href="agent_details.php?agent_code=<?=$row[agent_code]?>&<?=$decode_time?>&pg=<?=$_GET[pg]?>&sub=1" type="button" class="btn btn-add btn-sm" ><i class="fa fa-pencil"></i></a>
<a href="<?=$log_url?>?did=<?=$decode_val?>&<?=$decode_time?>&pg=<?=$_GET[pg]?>" type="button" class="btn btn-danger btn-sm"  onclick="return confirm('Are you sure to block?')"><i class="fa fa-trash-o"></i> </a>
</td>
...
if($pg!=1)
print "<A HREF='agent_join_dtls.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='agent_join_dtls.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='agent_join_dtls.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
