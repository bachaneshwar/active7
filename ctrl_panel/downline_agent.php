...
$sql="SELECT * FROM `sales_agent` WHERE `agent_code`='$_POST[agent_code]'";
$res=mysqli_query($con,$sql);
$numspid=mysqli_num_rows($res);

if($numspid>0){			 
echo '<META HTTP-EQUIV="Refresh" Content="0; URL=downline_agent_report.php?agent_code='.$_POST[agent_code].'">';
}
else{
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$log_url.'?msg=Agent ID is invalid.">';
}
...
<i class="fa fa-list"></i> Downline Agent
</div>
</div>
...
<tr>
<td>Agent ID</td>
<td><input type="text" class="form-control" name="agent_code" style="width:300px" required /></td>
