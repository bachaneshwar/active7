...
$my->delete(member_message,id,$decode_id);
...
$upper=30;
$lower=$upper*($pg-1);
$sql="select * from `member_message`";
...
<i class="fa fa-list"></i> Messages Details 
</div>
</div>
...
<?php
$msgsql="SELECT * FROM `member_message`  ORDER BY `id` DESC limit $lower,$upper";
...
if($pg!=1)
print "<A HREF='inbox_messages.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='inbox_messages.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='inbox_messages.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
