...
$upper=50;
$lower=$upper*($pg-1);
$sql="select * from `ewallet` WHERE `rest_amt`<>'0'";
...
<i class="fa fa-list"></i> All Ewallet Debit
</div>
</div>
...
<th scope="col" class="rounded">IFSC Code</th>			
...
if($_POST[spid]==""){
$msgsql="select * from `ewallet` WHERE `rest_amt`<>'0' ORDER BY `id` ASC limit $lower,$upper";
}else{
$msgsql="select * from `ewallet` WHERE spid='$_POST[spid]' AND `rest_amt`<>'0'";
}
...
<td>
<form method="post" action="ewallet_debit_action.php">
<input type="hidden" name="spid" value="<?=$row[spid]?>" readonly />
<input type="text" name="rest_amt" style="width:60px;font-size:12px" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" />
...
if($pg!=1)
print "<A HREF='ewallet_debit.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='ewallet_debit.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='ewallet_debit.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
