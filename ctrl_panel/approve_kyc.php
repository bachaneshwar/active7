...
$lower=$upper*($pg-1);
if($_POST[spid]){
echo $sql="select * from `member` WHERE `kyc_updt`='1' AND `spid`='$_POST[spid]' AND `kyc_status`='1' ";
}else{
$sql="select * from `member` WHERE `kyc_updt`='1' AND `kyc_status`='1' ";
...
<i class="fa fa-list"></i> Approved KYC Details
</div>
</div>
...
<?php
if($_POST[spid]){
$msgsql="select * from `member` WHERE `kyc_updt`='1' AND `kyc_status`='1'  AND `spid`='$_POST[spid]'  ORDER BY `memid` ASC limit $lower,$upper";
}else{
$msgsql="select * from `member` WHERE `kyc_updt`='1' AND `kyc_status`='1'  ORDER BY `memid` ASC limit $lower,$upper";
}
...
if($pg!=1)
print "<A HREF='approve_kyc.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='approve_kyc.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='approve_kyc.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
