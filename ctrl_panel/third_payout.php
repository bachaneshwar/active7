...
if($ptdt<=$endt){
header("location:generate_payout2.php?msg=End Date is not less than correct time.");
}
...
<form action="third_action.php" method="post">
...
$sel1 = "select * from `oneindia_tree`  WHERE `join_day`<='$endt' AND `st`='1' ORDER BY `oneindia_tree_id` ASC";
...
$sel_bin_id = "select * FROM `oneindia_tree` where `spid`='$row_pay[spid]'" ;
...
$left_pt = $binary."L" ;
$right_pt = $binary."R" ;
$final_level = ($row_bin_id[level]+$rowlm_1[one_india_level]);
...
$sql_direct1 = "SELECT * FROM `oneindia_tree` WHERE `join_day`>='$stdt' AND `join_day`<='$endt' AND `binary` LIKE '$left_pt%' AND `level`<='$final_level'"; 
$res_direct1 = mysqli_query($con,$sql_direct1);
$num_direct1 = mysqli_num_rows($res_direct1);

$sql_direct2 = "SELECT * FROM `oneindia_tree` WHERE `join_day`>='$stdt' AND `join_day`<='$endt' AND `binary` LIKE '$right_pt%' AND `level`<='$final_level'"; 
$res_direct2 = mysqli_query($con,$sql_direct2);
$num_direct2 = mysqli_num_rows($res_direct2);

$num_direct=($num_direct1+$num_direct2);

$one_india_income=($num_direct*$rowlm_1[one_india_income]);
