...
$value_num=$my->second_num($con,slider,slider_id,$decode_id,st,1);

if($value_num>0){
$value_det=$my->total_row($con,slider,slider_id,$decode_id);
}
...
$delsql = "DELETE FROM `slider` WHERE `slider_id`='$decode_id'";
$delres=mysqli_query($con,$delsql);
$nurl="$log_url?pg=$_GET[pg]&lid=$dtls_link[sublink_id]&";
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'msg=Successfully Deleted">';	
 }
?>
...
$my->new_modify_val($con,slider,slider_image,$folder,$nurl,$_POST[subId]);
...
$my->insertval1($con,slider,slider_image,$folder,$nurl);
...
<label>Slider </label>
<input type="text" class="form-control" name="slider_name" placeholder="Enter Short description" value="<?=$value_det[slider_name]?>" required>
</div>



<div class="form-group">
<label>Slider Amount</label>
<input type="text" class="form-control" name="slider_amount" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"   value="<?=$value_det[slider_amount]?>" required>
</div>
 -->
...
<input type="hidden" name="old_img" value="<?=$value_det['slider_image']?>" />
<input type="hidden" name="subId" value="<?=$value_det['slider_id']?>" />
...
<tbody>
<?php
$msgsql="SELECT * FROM `slider`  ORDER BY  `slider_id` ASC limit $lower,$upper";
$msgres=mysqli_query($con,$msgsql);
while($msgrow=mysqli_fetch_array($msgres)){
$c++;

$decode_val=base64_encode($msgrow[slider_id]);
...
<!-- <td><?=$msgrow[slider_name]?></td>
<td><?=$msgrow[slider_amount]?></td> -->

<!--<td><?=$pmsg?></td>
<td><?=$msgrow[self]?></td>
<td><?=$msgrow[direct]?></td>
-->
<td><? if($msgrow[slider_image]!=""){?><img src="../<?=$msgrow[slider_image]?>" style="width:100px;height:100px" /><?}?></td>
...
if($pg!=1)
print "<A HREF='slider.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='slider.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='slider.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
