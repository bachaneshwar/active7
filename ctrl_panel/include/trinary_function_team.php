...
function tree_create($con,$v,$table,$id,$date,$join_time,$sponsor){

$vsql43="SELECT * FROM `$table` WHERE `spid`='$sponsor'";
$vres43=mysqli_query($con,$vsql43);
$vrow43=mysqli_fetch_array($vres43);
$vlvl1=$vrow43[level]+1;
$vlvl2=$vrow43[level]+2;
...
$sql46="SELECT * FROM `$table` WHERE `binary` LIKE '$vrow43[binary]%' ORDER BY `$id` ASC LIMIT 1";
...
$newsql1="SELECT * FROM `$table` WHERE `level`='$vlvl1' AND `binary` LIKE '$vrow43[binary]%'  ORDER BY `$id` ASC";
...
$_SESSION[level47]=$vlvl2;
unset($_SESSION[level46]);
unset($newmem1);
}
if($_SESSION[level47]!=""){
for($i=$vlvl2;$i<=$_SESSION[level47];$i++){
$newsql1="SELECT * FROM `$table` WHERE `level`='$i' AND `binary` LIKE '$vrow43[binary]%'  ORDER BY `$id` ASC";
$newres1=mysqli_query($con,$newsql1);
while($newrow1=mysqli_fetch_array($newres1)){
...
$newsql51="SELECT * FROM `$table` WHERE `level`='$tab' AND `binary` LIKE '$vrow43[binary]%'";
