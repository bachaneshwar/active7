...
$value_num=$my->second_num($con,country,country_id,$decode_id,st,1);

if($value_num>0){
$value_det=$my->total_row($con,country,country_id,$decode_id);
}
...
$value_det=$my->total_row($con,country,country_id,$decode_id);
if($value_det[st]==1){
$my->normal_UPDATE1($con,country,st,0,country_id,$decode_id);
}else{
$my->normal_UPDATE1($con,country,st,1,country_id,$decode_id);
}
...
$my->new_modify($con,country,$nurl,$_POST[subId]);
$nurl="$log_url?pg=$_GET[pg]&lid=$dtls_link[sublink_id]";
echo '<META HTTP-EQUIV="Refresh" Content="0; URL='.$nurl.'">';		
}else{
$nurl="$log_url?lid=$dtls_link[sublink_id]";
$my->new_insert($con,country,$nurl);
}
...
<input type="text" class="form-control" name="country_name" placeholder="Enter Short description" value="<?=$value_det[country_name]?>"  onkeyup="showResult(this.value)"  required>
...
<input type="hidden" name="subId" value="<?=$value_det['country_id']?>" />
...
<tbody>
<?php
$msgsql="SELECT * FROM `country`  ORDER BY  `country_name` ASC limit $lower,$upper";
$msgres=mysqli_query($con,$msgsql);
while($msgrow=mysqli_fetch_array($msgres)){
$c++;

$decode_val=base64_encode($msgrow[country_id]);
...
if($pg!=1)
print "<A HREF='country.php?pg=$pre&lid=$dtls_link[sublink_id]'class='previous-off'>&laquo; Previous</A>";
else
...
{
if($i!=$pg)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='country.php?pg=$i&lid=$dtls_link[sublink_id]'>$i</A>";
else
print "&nbsp;&nbsp;&nbsp;&nbsp;$i";
}

if($pg!=$pgno)
print "&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='country.php?pg=$nxt&lid=$dtls_link[sublink_id]'class='next'>Next &raquo;</A>";
else
